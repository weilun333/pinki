...
CREATE TABLE `daily_check_in_config` (
  ...
  `level_no` int unsigned NOT NULL COMMENT '等級編號',
...
CREATE TABLE `daily_check_in_log` (
  ...
  `member_no` int NOT NULL COMMENT '會員編號',
  `recieve_point` int NOT NULL COMMENT '領取點數',
  `total_recieve_day` int NOT NULL COMMENT '領取天數 (累積)',
  `process_user` varchar(20) NOT NULL COMMENT '最後更新者',
  `last_update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最後更新時間 ',
  PRIMARY KEY (`check_in_no`),
...
CREATE TABLE `delivery_fee` (
  `deliever_no` int NOT NULL AUTO_INCREMENT COMMENT '運費編號',
  `brand_no` int NOT NULL COMMENT '品牌編號',
  `prod_size` int NOT NULL COMMENT '商品體積',
  `deliever_fee` int NOT NULL COMMENT '運費金額',
  `free_delivery_amount` int NOT NULL COMMENT '免運金額',
  PRIMARY KEY (`deliever_no`)
...
CREATE TABLE `member_level` (
  ...
  `one_time_comsumption` int NOT NULL COMMENT '單次消費金額',
  `process_user` varchar(20) NOT NULL COMMENT '最後更新者',
  `last_update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最後更新時間 ',
...
CREATE TABLE `prod_info` (
  ...
  `prod_specfication` varchar(200) NOT NULL COMMENT '商品規格',
...
CREATE TABLE `prod_order` (
  ...
  `prod_deliever_time` timestamp NULL DEFAULT NULL COMMENT '出貨時間',
  ...
  `receiever_name` varchar(20) NOT NULL COMMENT '收件者姓名',
  `reciever_tel` varchar(20) NOT NULL COMMENT '收件者電話',
  `receiver_mail` varchar(100) NOT NULL COMMENT '收件者mail',
  `reeceiver_addr` varchar(100) NOT NULL COMMENT '收件地址',
...
-- Dump completed on 2023-08-18 22:56:02
